...
echo "1. Checking for SSL files in /opt/vibestack-ansible/:"
sudo ls -la /opt/vibestack-ansible/ssl* 2>/dev/null || echo "No SSL files found"
echo ""

echo "2. Checking certificate validity:"
if [ -f /opt/vibestack-ansible/ssl.cert ]; then
    sudo openssl x509 -in /opt/vibestack-ansible/ssl.cert -noout -text | head -20
else
...
if [ -f /opt/vibestack-ansible/ssl.key ]; then
    sudo openssl rsa -in /opt/vibestack-ansible/ssl.key -check -noout 2>&1
else
...
sudo ls -la /opt/vibestack-ansible/*.b64 2>/dev/null || echo "No .b64 files found (good - they should be cleaned up)"
EOF
